...
contract RoleTestToken is ERC20, AccessControl {
    ...
    function addRoleToAccount(bytes32 role, address account) onlyRole(ADMIN_ROLE) public {
        _grantRole(role, account);

    }

    function removeRoleFromAccount(bytes32 role, address account) onlyRole(ADMIN_ROLE) public {
    ...
    function sendMessageToEveryone() public view returns (string memory){
        require(
            hasRole(ADMIN_ROLE, msg.sender) || hasRole(MODERATOR_ROLE, msg.sender) || hasRole(MINTER_ROLE, msg.sender) || hasRole(USER_ROLE, msg.sender),
